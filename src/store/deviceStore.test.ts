...
mock.module("../utils/storage", () => ({
  loadDevices: mock(() => Promise.resolve(null)),
  saveDevices: mock(() => Promise.resolve()),
}));

mock.module("../utils/inspector", () => ({
  inspector: undefined,
}));

const { useDeviceStore } = await import("./deviceStore");

const mockMachine = createMachine({ id: "mock", initial: "idle", states: { idle: {} } });

function createMockActor() {
  // Cast needed because the store expects a DeviceActor union type
  return createActor(mockMachine) as unknown as ReturnType<typeof createActor>;
}
